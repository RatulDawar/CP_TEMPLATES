...
vector<vector<ll>> matrix_multiply(vector<vector<ll>> &a,vector<vector<ll>> &b,ll mod){
    ...
    int n2 = b.size();
    int m2 = b[0].size();
    vector<vector<ll>> prod(n1,vector<ll>(m2,0));
    for(int i = 0;i<n1;i++){
        for(int j = 0;j<m2;j++){
            for(int k = 0;k<n2;k++){
...
vector<vector<ll>> expo(vector<vector<ll>> &a,ll b,ll mod){
    int n = a.size();
    int m = a[0].size();
...
void solve(){
    ll n;cin>>n;
    vector<vector<ll>> v{{19,7},{6,20}};
    vector<vector<ll>> ans = expo(v,n,MOD);
    cout<<ans[0][0];
}
int main()
{
